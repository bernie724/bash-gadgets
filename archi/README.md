...
```sh
curl -s -L https://github.com/bernie724/bash-gadgets/raw/main/archi.tgz > archi.tgz
tar xzpf archi.tgz
cd archi/; ./archi.sh 
#This will have a final warning, and then start and complete the install
```
##### Method #2
Copy and unpack the archi.tgz (above) to a booted Arch Install ISO and run archi.sh  
```sh
#from a local host
...
I would review and leave the settings for the first run, the config file is archi/archi.conf and the install sets are located @ archi/forsys/sets
```sh
curl -s -L https://github.com/bernie724/bash-gadgets/raw/main/archi.tgz > archi.tgz
tar xzpf archi.tgz
cd archi/
...
|Step  | Script          | Info                                                       |
| ---- | ------          | ----                                                       |
| 1    | archi.sh        | this is the install command, format/linux/core/sanity      |
